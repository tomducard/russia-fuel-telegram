"""Telegram scraping helpers using Telethon."""
...
def load_channels_csv(path: str | Path) -> List[str]:
    """Load channel usernames/URLs from a CSV with a 'channel' column."""
    df = pd.read_csv(path)
...
class TelegramScraper:
    ...
    def scrape_to_parquet(self, channels: Iterable[str], output_path: str | Path, limit: int = 500, min_date: datetime | None = None) -> Path:
        """Scrape and persist raw messages to Parquet, with incremental chunk saving."""
            ...
            for channel in tqdm(list(channels), desc="Channels"):
                ...
                try:
                    ...
                    try:
                         ...
                         tqdm.write(f"⚠️ Channel found but invalid/private: {channel}")
                         continue
                    except Exception as e:
                         tqdm.write(f"⚠️ Error resolving {channel}: {e}")
                         continue
                    ...
                    if records:
                        ...
                        tqdm.write(f"✅ Saved {len(records)} msgs from {channel}")
                    else:
                        tqdm.write(f"ℹ️ No messages found for {channel} (within limits)")
